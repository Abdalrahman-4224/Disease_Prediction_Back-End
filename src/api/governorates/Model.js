...
function check(id, existed) {
    var sql = `select id from governorates where deleted_at is NULL and id = ${id} `
...
function _insert(creator_id, recordData, callback) {
    try {
        ...
        var sql = `INSERT INTO governorates ( name, code, enabled, created_at, created_by_id)
...
async function _update(data, viewDisabledItems, req, callback) {
    ...
    var sql = `update governorates set `
    ...
    try {
        ...
        if ((enabled === 1 || enabled === 0) && (parseInt(itemWithOldData?.enabled) != enabled)) {
            ...
            activityNote += ` تم تغيير النوع اللوحة  من ${itemWithOldData?.enabled} الى ${enabled}`
            sql += ` enabled = ${enabled} ,`
        }
    } catch (error) {
        console.log("error in update governorate", error);
    }
    ...
    executeQuery(sql, 'updateGovernorate', async result => {
...
function _checkCodeOrIdExisting(code, id, viewDisabledItems, existed) {
    var sql
    if (code) sql = `select code from governorates where code = "${code}" `
    else sql = `select id from governorates where id = ${id} `
...
function _get(id, viewDisabledItems, callback) {
    ...
    var sql = `SELECT ${columns} from governorates  where deleted_at is null  and id = ${id}`
    if (!viewDisabledItems) sql += ` and enabled= 1`

    executeQuery(sql, 'getGovernorate', result => {
        if (result?.length > 0) {
...
async function _gets(searchParam, viewDisabledItems, callback) {
    ...
    var sql = `SELECT ${columns} from governorates  `
    var sqlFilters = ` where governorates.deleted_at is null  `
    if (!viewDisabledItems) sqlFilters += ` and governorates.enabled= 1`

    if (searchParam?.name) {
        sqlFilters += ` and governorates.name LIKE '%${searchParam?.name}%' `
    }
    if (searchParam?.code) {
        sqlFilters += ` and governorates.code LIKE '%${searchParam?.code}%' `
    }

    const checkEnabled = parseInt(searchParam?.enabled);
    if (checkEnabled === 1 || checkEnabled === 0) {
        sqlFilters += ` and governorates.enabled = ${checkEnabled} `
    }



    if (searchParam?.creation_date_from) {
        sqlFilters += ` and governorates.created_at >=  '${searchParam?.creation_date_from}' `
    }
    if (searchParam?.creation_date_to) {
        sqlFilters += ` and governorates.created_at <=  '${searchParam?.creation_date_to}' `
    }
    ...
    if (searchParam?.page && searchParam?.page_size) {
        ...
        let resultTotal = await new Promise((resolve) => {
            executeQuery("SELECT count(*) as total_records from governorates  " + sqlFilters, 'check', (e) => { resolve(e); });
        })
        ...
        sqlFilters += ' ORDER BY governorates.`created_at` DESC LIMIT ' + ((searchParam?.page - 1) * searchParam?.page_size) + ',' + searchParam?.page_size
    }
    sql += sqlFilters


    executeQuery(sql, 'getGovernorates', result => {
        if (result) {
            if (searchParam?.page && searchParam?.page_size) {
...
function _delete(data, req, callback) {
    ...
    check(id, async existed => {
        if (existed) {
            ...
            var sql = `update governorates set deleted_at = "${getDateTime()}", deleted_by_id = "${data?.id}" where id= ${id}`
...
const _import = async (data, req, callback) => {
    ...
    try {
        ...
        let insertQuery = `INSERT INTO governorates(
            name, 
            code, 
            enabled,
            created_at,
        ...
        for (let index = 1; index < dataToImport?.length; index++) {
            ...
            let query = `SELECT 
                id from governorates where name = '${row[0]}' or code = '${row[1]}'
            `
